...
export const REASON_OPTIONS: { value: AircraftAvailability["reason"]; label: string; icon: string }[] = [
...
export const mapEnglishToUI = (engDay: WeekDay): string => {
    switch (engDay) {
...
export const mapUIToEnglish = (uiDay: string): WeekDay => {
    switch (uiDay) {
...
export default function EventRecurrenceScreen() {
    ...
    const handleSave = async () => {
        ...
        const mappedDaysOfWeek = selectedDays.map(mapUIToEnglish) as WeekDay[];
    ...
    return (
        ...
        >
            <ThemedView className="flex-1 px-4 pt-2">
                {/* Header */}
                <AircraftDetailsHeader header="Añadir Indisponibilidad" />
