...
export function usePilotReservations(pilotId: string | undefined) {
  return useQuery<ClientReservationItem[]>({
    queryKey: ["pilot-reservations", pilotId],
    queryFn: async () => {
      if (!pilotId) return [];

      try {
        // Query by pilot_ids array containing current pilotId
        const q1 = query(
          collection(db, "aircraft-reservation"),
          where("pilot_ids", "array-contains", pilotId)
        );

        let snapshot = await getDocs(q1);

        // Fallback: query by pilots_ids if snapshot empty
        if (snapshot.empty) {
          try {
            const q2 = query(
              collection(db, "aircraft-reservation"),
              where("pilots_ids", "array-contains", pilotId)
            );
            snapshot = await getDocs(q2);
          } catch (e) {
            console.error("Error querying pilots_ids fallback:", e);
          }
        }
        ...
        for (const docSnap of snapshot.docs) {
          ...
          items.push({
            ...
            fa_flight_id: data.fa_flight_id || null,
            ...
            pilot_ids: data.pilot_ids || data.pilots_ids || [],
            created_at,
            aircraftSpecs,
            originAirport,
            destinationAirport,
        ...
        console.error("Error fetching pilot reservations:", error);
        return [];
      }
    },
    enabled: !!pilotId,
